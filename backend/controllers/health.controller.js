...
export const createHealth = async (req, res, next) => {
  try {
    ...
    const doc = await Health.create({
      cow, date: d, type, temperatureC, weightKg, symptoms, diagnosis, medication, dosage, vet,
      nextDueDate: nextDueDate ? startOfDay(nextDueDate) : undefined, notes, recordedBy
    });
...
export const listHealth = async (req, res, next) => {
  try {
    ...
    const [items, total] = await Promise.all([
      Health.find(q).populate("cow","name tagId breed gender").sort({ date: -1, _id: -1 }).skip(skip).limit(Number(limit)),
...
export const updateHealth = async (req, res, next) => {
  try {
    ...
    if (payload.nextDueDate) payload.nextDueDate = startOfDay(payload.nextDueDate);

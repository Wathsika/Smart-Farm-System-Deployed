...
const TaskModal = ({ show, onClose, onSave, onDelete, users, existingTask, preselectedDate, readOnly }) => { // Added readOnly prop
  ...
  const validateForm = () => {
    ...
    if (!task.title.trim()) {
      ...
      isValid = false;
    } else if (task.title.trim().length < 8) { // Changed from 5 to 8
      errors.title = "Task Title must be at least 8 characters."; // Updated message
      isValid = false;
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            required
            maxLength={200}
            minLength={8} // Changed from 5 to 8
            disabled={readOnly} // Disabled if in readOnly mode
          />
